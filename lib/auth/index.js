        ...
        .then(function (res) {
            if (typeof res === "object") {
                auth.prototype.getUser(res.account)
                    .then(function (res) {
                        auth.prototype.getGroup(res.user.getGroupId())
                            .then(function (res) {
                                if (auth.prototype.getCommand().getValue() <= res.group.getPower()) {
                                    d.resolve({err: null, auth: true});
                                } else {
                                    d.reject({
...
auth.prototype.getGroup = function (unknown) {
    ...
    case 'name':
        ...
        d.reject({err: null, message: 'Error: No group found'});
        break;
    case 'id':
        for (var i = 0, len = auth.__groups.length; i < len; i++) {
        ...
        .then(function (res) {
            ...
            for (var key in keys) {
                ...
                case 'power':
                    ...
                    break;
                default:
                    d.reject({err: true, message: 'Error: options may only contain the following keys: power'});
                    break;
                }
                i++;
            }
            ...
            // Options are valid, we can now attempt to update the record
            db.serialize(function () {
                console.log(_.values(options).concat(group.getId()));
                db.run(query, _.values(options).concat(group.getId()), function (err) {
                    console.log(err);
                    if (err === null) {
                        for (var key in keys) {
                            group.setProperty(key, options[key]);
                        }
                        d.resolve({err: null, message: 'User Updated Successfully!'});
                    } else {
                        d.reject({err: err, message: 'Failed to update user'});
    ...
    db.serialize(function () {
        db.run("INSERT INTO command_permissions (command, value) VALUES(?, ?)", [command, value], function (err) {
            if (err === null) {
                ...
                // If Error assume unique collision and return error
                d.reject({err: err, message: 'This command is already restricted'});
            }
        });
    });
    return d.promise;
