...
auth.prototype.init = function () {
    ...
    auth.prototype.createTables()
        ...
        .then(auth.prototype.fetchCommandsFromDb)
        .then(function () {
                ...
                auth.prototype.getUser(res.account)
                    .then(function (res) {
                        auth.prototype.getGroup(res.user.getGroupId())
                            .then(function (res) {
                                if (auth.prototype.getCommand().getPower() <= res.group.getPower()) {
                                    d.resolve({err: null, auth: true});
                                } else {
                                    d.reject({
...
auth.prototype.isCommandRegistered = function (command) {
    var commandObject = auth.__commands[command];
...
auth.prototype.getcommand = function (unknown) {
    ...
    case 'command':
        for (i = 0, len = auth.__commands.length; i < len; i++) {
            name = auth.__commands[i].getCommand();
            if (name.length > 1) {
                // Resolve if found
...
auth.prototype.fetchCommandsFromDb = function () {
    var d = when.defer(),
        commands = [];
    db.serialize(function () {
