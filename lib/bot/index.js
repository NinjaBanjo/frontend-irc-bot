...
var Bot = module.exports = function (config, pluginLoader) {
...
Bot.prototype.registerCommand = function (command, callback) {
...
Bot.prototype.listeners = {
  ...
  message: function (clientId, args) {
    ...
    if (reg.test(text)) {
      var command = text.match(/^![a-zA-Z0-9-_\/]+ ?/)[0].trim().replace(/^!/, '');
      var parameters = text.split(/^![a-zA-Z0-9-_\/]+ /, 2)[1];
      ...
      if (typeof matchedCommand === "object") {
        matchedCommand.callback.apply(this, client.concat(parameters, additionalArguments));
      }
    }
  }
};
