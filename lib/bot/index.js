...
Bot.prototype.init = function (authLib) {
    var self = this;
    auth = authLib;
    for (var server in this.__config.servers) {
        if (server.length > 50) {
            throw new Error('Server name has to be 50 chars or less');
            process.exit(1);
        }
        var serverConfig = this.__config.servers[server],
            client = new irc.Client(serverConfig.host, serverConfig.nick, {
        ...
        var newServer = new Server(client, server);
        ...
        (function (server) {
            ...
            newServer.getClient().addListener('registered', function () {
                // Pass the server id to the callback
                Bot.prototype.identify.call(self, server);
                Bot.prototype.joinChannels.call(self, newServer);
            });
            // Bind the message listener for all channels
            server.getClient().addListener('message', self.bindListener.call(self, 'message', newServer));
        })(newServer);
        // bind pm listener
        newServer.getClient().addListener('pm', function (from, message) {
            Bot.prototype.log(from + ' => ME: ' + message);
        });
        // Bind error listener
        newServer.getClient().addListener('error', function (err) {
            Bot.prototype.log(err);
        });
        // Connect to the IRC network
        Bot.prototype.log.call(this, "Connecting...");
        newServer.getClient().connect(5);
        // Store server in the list
        this._servers[server] = newServer;
    }
};

Bot.prototype.joinChannels = function (server) {
    var self = this;
    Bot.prototype.log.call(this, 'Joining Channels...');
    for (var i = 0, len = this.__config.servers[server.getName()].channels.length; i < len; i++) {
        // Closure is used so channel is available in the callback
        (function (channel) {
            server._client.join(self.__config.servers[server.getName()].channels[i], function () {
                Bot.prototype.log('Channel ' + channel + ' Joined');
            });
        })(this.__config.servers[server.getName()].channels[i]);
    }
};

Bot.prototype.identify = function (server) {
    Bot.prototype.log('Authenticating...');
    server._client.say('nickserv', 'identify ' + this.__config.servers[server.getName()].password);
...
Bot.prototype.listeners = {
    ...
    message: function (server, args) {
        var client = server._client,
        ...
        if (reg.test(text) && text != this.__trigger) {
            var command = text.match(new RegExp("^" + this.__trigger + "[a-zA-Z0-9-_\/]+ ?"))[0].trim().replace(reg, ''),
                ...
                matchedCommand = Bot.prototype.getCommand.call(this, client, command);
            (function (parameters) {
                auth.authorize(server._client, command, additionalArguments[0])
                    .then(function (res) {
                        if (typeof res === "object" && res.auth === true) {
                            ...
                            if (typeof matchedCommand === "object") {
                                ...
                                matchedCommand.callback.apply(this, [server._client].concat(command, parameters, additionalArguments));
                            }
                        } else {
...
Bot.prototype.getClient = function (server) {
