...
Bot.prototype.init = function () {
    ...
    for (var networkName in this.__config.networks) {
        ...
        (function (newNetwork) {
            ...
            newNetwork.getClient().addListener('registered', function () {
                // Pass the server id to the callback
                Bot.prototype.identify.call(self, newNetwork);
                Bot.prototype.joinChannels.call(self, newNetwork);
            });
            // Bind the message listener for all channels
            newNetwork.getClient().addListener('message', self.bindListener.call(self, 'message', newNetwork));
        })(newNetwork);
        // bind pm listener
...
Bot.prototype.listeners = {
    message: function (network, args) {
        ...
        if (reg.test(text) && text != this.__trigger) {
            ...
            (function (parameters) {
                network.auth.authorize(command, additionalArguments[0])
                    .then(function (res) {
                        if (typeof res === "object" && res.auth === true) {
                            ...
                            if (typeof matchedCommand === "object") {
                                ...
                                matchedCommand.getCallback().apply(matchedCommand.getContext(), [network].concat(command, parameters, additionalArguments));
                            }
                        } else {
